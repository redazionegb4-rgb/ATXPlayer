...
struct MovieDetailView: View {
    ...
    private var castNames: [String] {
        (info?.cast ?? item.cast ?? "").split(separator: ",").map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }.filter { !$0.isEmpty }.prefix(10).map(String.init)
    }
...
struct SeriesDetailView: View {
    ...
    private var seriesCast: [String] {
        (info?.info?.cast ?? item.cast ?? "").split(separator: ",").map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }.filter { !$0.isEmpty }.prefix(10).map(String.init)
    }
